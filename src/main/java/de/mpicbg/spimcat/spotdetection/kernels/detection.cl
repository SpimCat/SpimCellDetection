...
)
{
    ...
    float localMin = READ_IMAGE(input, sampler, pos).x + 1;
    ...
    for(int x = -radius; x < radius + 1; x++)
    {
        for(int y = -radius; y < radius + 1; y++)
        {
            for(int z = -radius; z < radius + 1; z++)
            {
                ...
                float value = READ_IMAGE(input, sampler, localPos).x;
    ...
    if ((detect_maxima == 1 && pos.x == localMaxPos.x && pos.y == localMaxPos.y && pos.z == localMaxPos.z) ||
        ...
        WRITE_IMAGE(output, pos, (DTYPE_OUT){1, 0, 0, 0});
    } else {
        WRITE_IMAGE(output, pos, (DTYPE_OUT){0, 0, 0, 0});
    }
}