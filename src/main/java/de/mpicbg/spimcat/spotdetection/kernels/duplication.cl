__kernel void crop(write_only image3d_t dst, read_only image3d_t src) {
...
__kernel void copy(write_only image3d_t dst, read_only image3d_t src, float start_x, float start_y, float start_z) {
  const sampler_t sampler = CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP_TO_EDGE | CLK_FILTER_NEAREST;
